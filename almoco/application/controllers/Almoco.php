...
class Almoco extends MY_Controller {
    ...
    function cardapio() {
        ...
        $this->load->view('tela_auxiliar\cardapio.aux.php', $data);
        $this->load->view('rodape', $data);
    }
    ...
    function deletar_cardapio($id) {
        $dados_dia = '<label for="nome">Dia:</label><br/>';
        foreach ($dados['dia_semana'] as $dia):
            $dados_dia .= $dia->dia_semana . ':<input type="checkbox" name="dia_semana[]" value="' . $dia->id . '"' . set_checkbox('dia_semana[]', 'Segunda') . '/>';
        ...
        $dados['campo'] = 'id';
        if ($this->model->deletar_do_sistema($dados)) {
            $dados2['id'] = $id;
            $dados2['tabela'] = 'cardapio_x_dia';
            $dados2['campo'] = 'id_cardapio';
            $this->model->deletar($dados2);
            redirect('almoco/cardapio');
        } else {
            log_message('error', 'Erro ao deletar a pessoa.');
        }
    ...
    public function dados_cardapio() {
        $dados = $this->model->buscar_cardapio($this->input->post('id')) ;
      
        $dados_dia = '<label for="nome">Dia:</label><br/>';
        foreach ($dados as $dia):
            $dados_dia .= $dia->id . ':<input type="checkbox" name="dia_semana[]" value="' . $dia->id . '"' . set_checkbox('dia_semana[]', 'Segunda') . '/>';
        endforeach;
        $array_clientes = array(
            "dia" => $dados_dia,
            "cardapio"=>$dia->cardapio
        );
